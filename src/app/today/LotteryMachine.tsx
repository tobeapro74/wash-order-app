...
// ElementId → 해당 영상 파일
const VIDEO_MAP: Record<ElementId, string> = {
  face:  "/lottery-machine-세수.mp4",
  hair:  "/lottery-machine-머리.mp4",
  body:  "/lottery-machine-샤워.mp4",
  teeth: "/lottery-machine-양치.mp4",
};

const POSTER_MAP: Record<ElementId, string> = {
  face:  "/lottery-machine-세수-poster.jpg",
  hair:  "/lottery-machine-머리-poster.jpg",
  body:  "/lottery-machine-샤워-poster.jpg",
  teeth: "/lottery-machine-양치-poster.jpg",
};
...
export default function LotteryMachine({
  ...
  // 현재 스텝의 ElementId (영상 src 결정용)
  const [currentId, setCurrentId] = useState<ElementId>("teeth");
  ...
  const handleSpin = () => {
    ...
    if (stepRef.current === 0) {
      const order = makeShuffledOrder();
      fullOrder.current = order;
      setPicked([]);
      setCurrentId(order[0]);
    } else {
      setCurrentId(fullOrder.current[stepRef.current]);
    }

    setPhase("playing");

    // src가 바뀐 뒤 한 틱 대기 후 load → play
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", width: "100%", aspectRatio: "1/1", background: "#c8e8d4" }}>
        <video
          ref={videoRef}
          src={VIDEO_MAP[currentId]}
          poster={POSTER_MAP[currentId]}
          muted
          playsInline
          preload="auto"
          aria-hidden="true"
